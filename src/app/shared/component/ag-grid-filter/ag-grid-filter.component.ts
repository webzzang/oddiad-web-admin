...
@Component({
  ...
  templateUrl: './ag-grid-filter.component.html',
  styleUrls: ['./ag-grid-filter.component.scss']
})
export class AgGridFilterComponent implements OnInit {
  ...
  constructor(private service: AgGridFilterService,
    private spinner: NgxSpinnerService,
    private confirm: ConfirmService,
    private lang: TranslateService) {}

  ngOnInit() {
    this.dataEmptyText = this.lang.instant("common.data-empty");
    ...
    if(this.defaultColDef){
      ...
      this.defaultColDef = {
        ...
        sortable : true,
        headerComponentParams: { gridId: this.gridId, useSort: false }
      }
    }
  ...
  private getApiList(params?) {
    if(this.api){
      ...
      this.service.getList(this.api, this.apiParams).subscribe(res => {
        ...
        this.spinner.hide(this.gridId);
        // this.agGrid.hideOverlay();
        console.log("# ag-grid-filter- err -> ", err);
        if(err.body){
          if(err.body.message){
      ...
      console.log("# ag-grid-filter- api is empty");
    }
  }
