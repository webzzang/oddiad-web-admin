 ...
 */
@Component({
  selector: 'app-sitetopbanner-list',
  templateUrl: './sitetopbanner-list.component.html',
  styleUrls: ['./sitetopbanner-list.component.scss']
})
export class SitetopbannerListComponent extends CommonComponent implements OnInit {
  ...
  private init() : void {
    this.grid = {
      api: ContentService.URL.SEARCH_TOP_BANNER_LIST,
      isInitLoad: false,
      rowClassRules: {},
      columnDefs: [
        { headerName: '배너 이름', field: 'name', width: 150, sortable: false, useSort: false,
          ...
          cellRendererParams: {
            onClick: this.moveTopBannerMng.bind(this)
          }
        },
    ...
    if (this.possibleRegister(true)) {
      this.grid.funcButtons = [
        {
          label: '상단배너 등록',
          clickEventHandler: this.moveTopBannerMng.bind(this)
        }
      ];
    }
    ...
    this.search = {
      ...
      searchBtnHandler: ($event) : void => {
        ...
        this.searchTopBannerList(this.search.params);
      },
   ...
   * @private
   */
  private searchTopBannerList(params?: any): void {
   ...
   * @private
   */
  private moveTopBannerMng(event) : void {
