 ...
 */
@Component({
  selector: 'app-envbottombanner-list',
  templateUrl: './envbottombanner-list.component.html',
  styleUrls: ['./envbottombanner-list.component.scss']
})
export class EnvbottombannerListComponent extends CommonComponent implements OnInit {
  ...
  private init() : void {
    this.grid = {
      api: ContentService.URL.SEARCH_BOTTOM_BANNER_LIST,
      isInitLoad: false,
      rowClassRules: {},
      columnDefs: [
        { headerName: '배너 이름', field: 'name', width: 150, sortable: false, useSort: false,
          ...
          cellRendererParams: {
            onClick: this.moveBottomBannerMng.bind(this)
          }
        },
    ...
    if (this.possibleRegister(true)) {
      this.grid.funcButtons = [
        {
          label: '하단배너 등록',
          clickEventHandler: this.moveBottomBannerMng.bind(this)
        }
      ];
    }
    ...
    this.search = {
      ...
      searchBtnHandler: ($event) : void => {
        ...
        this.searchBottomBannerList(this.search.params);
      },
   ...
   * @private
   */
  private searchBottomBannerList(params?: any): void {
   ...
   * @private
   */
  private moveBottomBannerMng(event) : void {
